...
outcome <- lm(birthweight_imputed ~ Group + BOP_change + Group:BOP_change + White + Nat.Am + Diabetes + cigday_imputed + Any.live.ptb.sb.sp.ab.in.ab + BL.PD.avg + I(BL.PD.avg^2) + BL.CAL.avg + I(BL.CAL.avg^2), data = opt_mediation)
...
# CDE
CDE <- outcome$coefficients[2] + outcome$coefficients[13]
CDE
...
NIE <- outcome$coefficients[3] * mediator$coefficients[2] +
  outcome$coefficients[13] * mediator$coefficients[2]
NIE
...
for (i in 1:B) {
  ...
  CDE.boot <- c(
    CDE.boot,
    outcome.boot$coefficients[2] + outcome.boot$coefficients[13]
  )
  NIE.boot <- c(
    NIE.boot,
